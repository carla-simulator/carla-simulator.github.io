...
There is no doubt about (ROS)[http://www.ros.org/] being the most used system to put together robotics solutions. Hundreds of teams use ROS around the world to create their autonomous agents. For this reason, we took very seriously the task of updating CARLA's ROS bridge to be fully compatible with the new 0.9.X branch. From 0.9.2 CARLA's ROS bridge will live in its own repository (here)[https://github.com/carla-simulator/ros-bridge]{:target="_blank"}.
