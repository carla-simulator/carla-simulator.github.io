...
CARLA's API now provides new functions to retrieve [__ground truth pedestrian 3D bone positions__](#pedestrian-ground-truth-skeletons) and hence evaluate the 3D pose, facilitating the evaluation and training of pose estimation models and also adding custom gestures to standard animations through the API. 

CARLA 0.9.13 presents new assets to enrich your simulations with a variety of vehicles and pedestrians [__4 new pedestrians__](#new-pedestrians). 4 new pedestrians are introduced including 2 adults (each with 3 variants) and 2 children! 2 [__new vehicles__](#new-vehicles) have been added to the CARLA garage, the __Volkswagen T2 2021__ and a new __Ford Crown Taxi__. All generation 2 (GEN-2) cars now include [__articulated doors__](#opening-doors) that can interact with the environment through collisions, specially useful for training and testing autonomous vehicles in car parks or on tight city streets. 
...
#Choose an object and modify its texture
...
Continued improvement of the useability of the CARLA Python API comes with access to the Python version 3.9 version of the library for Linux and Windows, available through the PIP package manager.

```sh
pip install carla #for Python Version 3.9
```
