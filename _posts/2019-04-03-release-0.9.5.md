...
description: "In this release, we've added many new features such as an overhauled Waypoints API, a new town (Town07), no-rendering mode improvements and numerous others."
...
This release adds many important features and improvements to CARLA. Let's take a detailed look at what's new in this release!
...
- Our new data structures are now more similar to OpenDrive, so the parser is more expandable and queries are easy.

#### Python API Changes

We always try to avoid changing the Python API where possible but in this case, this was not possible.
...
## Lane Change Support for Global Route Planner
...
Our content has a new home! All our assets have been moved from google drive to AWS and our build scripts have been updated to address this change. Our assets are now a lot more accessible. Instead of depending on the scripts to download them programmatically, the assets can now be accessed manually with the AWS link. The link to the assets for each release can be found in the ContentVersions text file in the carla/Util folder. 
