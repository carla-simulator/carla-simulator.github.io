...
subtitle: "Extended API, night mode, improvements on the traffic manager and new documentation"
description: "The Python API has been extended for a better usage of waypoints and junctions, now giving support to all types of OpenDRIVE geometries."
author: "@sergi-e"
...
Last but not least, new documentation is available and much more will be provided soon. Right now, the sections regarding the installation of CARLA, F.A.Q. and the Python API have been revisited. There is also new documentation for the traffic manager, and of course, the [CARLA forum](https://forum.carla.org/)! 
...
This upgrade includes to features that are completely new.  
Firstly, there is the fog, that can be managed using two parameters: density and distance. 
__Distance__ sets the meters for the fog to start being applied. From that starting point, the __density__ will regulate the opacity of that will slowly fade the further elements in scene. 

> Nieblita. 

There is also a new wetness parameter that states the humidity of the road. In comparison to the precipitation deposits previously existing, this slowly applies a fader to the road instead of creating different pods along it. 

> Visuals. 

Regarding the wind, in previous releases the wind speed only was evident when tree leaves moved. Not it also affects the rain direction, the clouds movement and other types of vegetation. 

> Visuals. 

Other visual effects have also been added. The sun intensity, regulated by the sun position and cloudiness, affects the intensity and color temperature of the light. Also, quite similar to this, the more cloudy the day is, the darker the sky will be. 

> Visuals. 

All of these can be perceived using a RGB camera sensor. The CARLA team also provides a script, `/Utils/Weather.py` that allows to set the weather parameters. Also, the previous dynamic weather in `/PythonAPI/examples` has been updated to also manage the new elements on its cycle. 
...
A whole new world in CARLA! The moment the sun disappears in the horizon (being so that the angle goes under 0), it is considered night time in this world. 
When night comes, the city lights are automatically turned on. There are different types of street lights available. 

> Visualiçaçao

__Warning:__ Be careful when playing around in night mode, as the complex lightning may cause FPS drops. As an estimation, by the time of the release every city is able to manage a hundred vehicles with no drops under the 20FPS. 

### **CAR lights**

Right now, only the cars managed by the user will be able to turn on their lights. These have been implemented for six car models. These are: Etron, Lincoln, Mustang, Tesla 3S, Wolkswagen T2 and the new guest coming to CARLA. Speaking of which... 

The Cybertruck is here! 

> Fotico bonita.

The lights included sum all of the common front and back lights in a car, and some special ones such as license plate or interior light. Some of them turn on automatically (brake and reverse lights), but most of them though are implemented to be accessed through the API. 

> Código de ejemplo
> Resultado 

The script provided for manual control in `/PythonAPI/examples` has also been updated to include commands for lights. Left/Right blinkers correspond to the `Z/X` keys. High beams are turned on while pressing `Shift+L` and the rest of the lights follow a cycle with the `L` key that follows the structure: `off>position light+license plate>low beam>fog lights`.

> Manual control? 


> __Exactamente qué luces son automáticas__
...
Using distance fields, the __Distance field shadows__ are enabled, creating diffuse shadow edges instead of hard ones. This work hand in hand with the previous method of Cascade shadows to get a much more desirable effect: 

> Visual antes y después. 

Also, the __Distance field ambient occlusion__ along with the previous Screen space ambient occlusion help placing the objects on scene by creating shadows that take into consideration the proximity between elements. 
