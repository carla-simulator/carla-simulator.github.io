...
```
> Code example 3: How to modify a parameter unsing the new API calls.
...
client.tick() 
```
> Code example 4: Setting synchornous mode and sending clock ticks.
...
```
actors = [x for x in world.world.get_actors() if x.id != world.player.id]
client.apply_batch([carla.command.ApplyVelocity(x.id, carla.Vector3D(z=5)) for x in actors])

batch = [carla.command.SetAutopilot(x.id, False) for x in actors]
batch += [carla.command.ApplyVehicleControl(x.id, carla.VehicleControl(throttle=1.0)) for x in actors]
...
for x in tl.get_group_traffic_lights():
    x.freeze()
    state = carla.TrafficLightState.Green if x.get_pole_index() == tl.id else carla.TrafficLightState.Red
    x.set_state(state)
```
...
for traffic_light in list_traffic_lights:
            if hasattr(traffic_light, 'trigger_volume'):
                ...
                s = Utils.length(traffic_light.trigger_volume.extent) + Utils.length(self._actor.bounding_box.extent)
                if distance <= s:
